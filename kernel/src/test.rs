        ...
        if session.is_none() {
            ...
            *session = Some(
                Arc::new(FuzzSession::from_falkdump(
                        "192.168.124.1:1911", "fuzzer.falkdump", |_worker| {
                    // Mutate the master at this point
                })
                .timeout(1_000_000)
                .vmexit_filter(vmexit_handler)
                .inject(inject)
                )
...
fn inject(worker: &mut Worker, _context: &mut dyn Any) {
    ...
    //cause crash
    worker.set_reg(Register::Rbx, 0x41414141);

    worker.fuzz_input = Some(input);
}

fn vmexit_handler(worker: &mut Worker, vmexit: &VmExit) -> bool{
    print!("vmexit called");
