...
lazy_static! {
    ...
    pub static ref WRITER: Mutex<Writer> = Mutex::new(Writer {
        ...
        buffer: unsafe { &mut *( (KERNEL_PHYS_WINDOW_BASE + 0xb8000) as *mut Buffer) },
    });
}
...
macro_rules! print_red {
    ($($arg:tt)*) => ($crate::print::_print_red(format_args!($($arg)*)));
}

/// Like the `print!` macro in the standard library, but prints to the VGA text buffer.
#[macro_export]
macro_rules! print_blue {
    ($($arg:tt)*) => ($crate::print::_print_blue(format_args!($($arg)*)));
}
